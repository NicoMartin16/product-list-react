...
const Cart = ({ productsSelected, onRemoveItem, onConfirmOrder }: CartProps) => {
      ...
      <div className="cart-container">
        {productsSelected && productsSelected.length > 0 ? (
          <>
            {productsSelected.map((item, index) => (
              <div className="cart-item" key={index}>
                <div className="cart-item-info">
                  ...
                  <p className="total-price rose-500">${getTotalPerProduct(item.price, item.quantity)}</p>
                </div>
                <div onClick={(e) => removeItemFromCart(e,index)} className="cart-item-remove">
                  <img
            ...
            <button onClick={handleConfirmOrder} className="button-confirm bg-red">Confirm order</button>
          </>
        ) : (
          <div>
